...
instance Controller EventsController where
    action EventsAction = do
        ...
        weeklyActiveProjectsPerOS <- sqlQuery "SELECT date, os, COUNT(distinct project_id) AS count FROM (SELECT date_trunc('week', events.created_at) AS date, project_id, os FROM events) AS events_with_date GROUP BY os, date" ()
